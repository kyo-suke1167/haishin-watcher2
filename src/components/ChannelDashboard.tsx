      ...
      <section className="bg-gray-900/50 p-6 rounded-xl shadow-2xl border border-emerald-900/50 mb-10 backdrop-blur-sm">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <button
            ...
            className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 px-8 rounded-lg transition-all shadow-lg shadow-emerald-900/20"
          >
            レーダーに追加
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {channels.length === 0 ? (
            ...
            channels.map((channel) => (
              <div
                key={channel.id}
                className="bg-gray-800 border border-gray-700 rounded-xl p-4 flex items-center justify-between hover:border-emerald-500/50 transition-colors group"
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  {channel.thumbnail ? (
                    <img
                      ...
                      alt={channel.name}
                      className="w-10 h-10 rounded-full object-cover shrink-0 border border-gray-600"
                    />
                  ) : (
